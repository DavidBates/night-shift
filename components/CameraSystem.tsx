...
export const CameraSystem: React.FC<CameraSystemProps> = ({ 
  ...
  return (
    <div className="fixed inset-4 z-40 bg-black border-4 border-slate-600 shadow-2xl flex rounded-lg overflow-hidden animate-in zoom-in duration-200">
        <div className="flex-1 relative bg-slate-900">
            <RoomView id={currentCam} animatronics={animatronics} cameraStatic={cameraStatic} />
        </div>
        
        {/* Map Sidebar */}
        <div className="w-72 bg-slate-800 border-l border-slate-600 relative p-4 flex flex-col">
            <div className="text-center text-white mb-4 border-b border-slate-600 pb-2 font-bold tracking-widest">CAMERA SYSTEM</div>
            <div className="relative h-64 w-full bg-slate-900 rounded border border-slate-700 opacity-90 shadow-inner">
                {/* Map Nodes */}
                <MapButton 
            ...
            <div className="mt-auto text-slate-500 text-xs space-y-2">
                 <div className="flex justify-between"><span>AUDIO:</span> <span className="text-green-500">ONLINE</span></div>
                 <div className="flex justify-between"><span>VIDEO:</span> <span className="text-green-500">ONLINE</span></div>
                 <div className="flex justify-between"><span>AI LEVEL:</span> <span className="text-red-500">{night * 2}/20</span></div>
            </div>
        </div>
    </div>
  );
